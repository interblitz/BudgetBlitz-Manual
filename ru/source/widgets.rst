...
Для изменения этих настроек служит кнопка :guilabel:`Настройка`.

В разделе :guilabel:`Вид` можно задать основные параметры виджета.

:guilabel:`Наименование` удобно использовать, если на экран выведено несколько виджетов. При желании можно оставить это поле
пустым.

:guilabel:`Типы портфелей`, :guilabel:`Портфели` и :guilabel:`Счета` служат для базового ограничения отображаемой в
...
:guilabel:`Отображать баланс` служит для включения и отключения расчета баланса, также можно конкретизировать как именно
...
Виджет содержит кнопку :guilabel:`Новая операция`. Эта кнопка доступна после того, как в настройках будет указан счет для
...
служат параметры расположенные в разделе :guilabel:`Фильтр`
...
Откроем настройки виджета и зададим название :samp:`Общественный транспорт`.
...
В настройках фильтра зададим статью :samp:`Общественный транспорт` и сохраним настройку.

.. image:: images/widgets-110-widget-example.png
  :width: 25%

Теперь виджет отображает движения только по статье :samp:`Общественный транспорт` за текущий месяц, мы видим
