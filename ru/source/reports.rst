...
из отчета видно, что по статье :dfn:`Доходы от аренды` прошло внеплановое поступление денежных средств, а по статье :dfn:`Зарплата`
...
из отчета видно, что по статье :dfn:`Доходы от аренды` было поступление денежных средств, по статье :dfn:`Карманные расходы`
было списание и т.д.
...
Отчет формируется по операциями, которые содержат статьи с признаком :dfn:`Суммируемая` и при этом являются
...
Так, например, из отчета видно, что по статье :dfn:`Кредиты (я должен)` на начало периода не было остатка. Затем
...
признаком :dfn:`Суммируемая` и являются либо доходными, либо расходными. Из суммы плановых операций вычитается
...
Так, например, из отчета видно, что по статье :dfn:`Зарплата` на начало периода есть остаток плана, т.е. фактическая сумма
движений по статье :dfn:`Зарплата` меньше запланированной. По этой статье также в течение выбранного периода запланировано
