...
После первого запуска отредактируйте основные настройки приложения. Для этого через меню |menu_actions_profile|
...
#. должен быть отмечен соответствующий флажок в активном профиле, меню |menu_actions_profile|;
#. приложению должен быть предоставлен доступ к SMS (Android 6 и выше).

Для автоматического импорта push-уведомлений в активном профиле, меню |menu_actions_profile|:

#. должны быть выбраны пакеты, уведомления которых |бб| будет импортировать:
#. должны быть предоставлены дополнительные разрешения.

Чтобы загрузить настройки Вашего банка нажмите кнопку |button_sms_import_settings|. В дальнейшем загрузить
настройки можно будет через меню |menu_actions_import_sms_import_settings| или из справочника
...
После установки программа содержит три типа портфелей |item_personal|, |item_small_business| и |item_universal|,
один персональный портфель |item_wallet|, два счета |item_card| и |item_cash| в портфеле |item_wallet| и предзаполненный
список категорий.
...
следует указать статью |item_category_initial_balance|.
...
следует указать статью |item_category_credit_limit|. Обратите внимание, что это техническая статья,
у нее отключены признаки |property_category_income| и |property_category_outcome|. Подробней причины ввода кредитного лимита операциями
...
#. ввести приходную (положительную) операцию на сумму 1000 |c| по статье |item_category_credit| и указать плательщика или персону --- кому должны.
#. ввести расходную (отрицательную) операцию на сумму 1000 |c| по статье |item_category_empty|, либо задать статью, если известно, на что были потрачены средства.
